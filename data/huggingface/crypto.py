...
CONFIG = {
    ...
    "api": {
        ...
        "top_n": 300,
        "rank_by": "volume", # volume or change
        "min_volume": 500000,
        "max_concurrent": 8,
        "kline_limit": 1000,
        "exclude_tokens": ["DAI", "EUR"]
    },
...
class DataEngine:
    ...
    async def _fetch_g_klines(self, session: aiohttp.ClientSession, symbol: str, interval: str) -> Optional[pd.DataFrame]:
    ...
    async def _fetch_o_klines(self, session: aiohttp.ClientSession, symbol: str, interval: str) -> Optional[pd.DataFrame]:
    ...
    async def _fetch_b_klines(self, session: aiohttp.ClientSession, symbol: str, interval: str) -> Optional[pd.DataFrame]:
...
class IndicatorEngine:
    ...
    @staticmethod
    def utb_indicator(df: pd.DataFrame, key_value: float = 2.0, atr_period: int = 14, use_ha: bool = False) -> pd.DataFrame:
    ...
    @staticmethod
    def stc_indicator(df: pd.DataFrame, stc_length: int = 12, stc_fast_length: int = 26, stc_slow_length: int = 50, stc_factor: float = 0.5) -> pd.DataFrame:
        src = df['close'].values
        n = len(src)
    ...
    @staticmethod
    def tma_indicator(df: pd.DataFrame, tma_fast_length: int = 5, tma_slow_length: int = 8, tma_vfactor: float = 0.618) -> pd.DataFrame:
...
class StrategyEngine:
    ...
    def execute(self, df: pd.DataFrame, symbol: str, interval: str, strategy_filter: Optional[List[str]] = None):
...
class NotifyEngine:
    ...
    def process_results(self, results: list, interval: str, label: str = ""):
        ...
        if self.cfg.get('console_enable'):
            logger.info(f"{label} Scan complete | Instruments monitored: {len(results_list)} | Signals triggered: {len(signals)}")
...
class ScanEngine:
    ...
    def _validate_intervals(self):
        router = self.cfg['strategy']['router']
        indicators = self.cfg['strategy'].get('indicators', {})
        missing = []

        for strategy_name, strategy_cfg in router.items():
            if not strategy_cfg.get('enabled'):
                continue
            intervals = strategy_cfg.get('intervals') or self.cfg.get('intervals', [])
            for interval in intervals:
                if interval.upper() not in indicators:
                    missing.append(f"{strategy_name} → {interval}")

        if missing:
            raise ValueError(
                "❌ Missing indicator configs for the following intervals, please add them and restart:\n" +
                "\n".join(f"  {m}" for m in missing)
            )

        logger.info("✅ Interval config validation passed")

    async def _proc_symbol(self, session: aiohttp.ClientSession, symbol: str, interval: str, sem: asyncio.Semaphore,
                           strategy_filter: Optional[List[str]] = None) -> Optional[Any]:
        async with sem:
            try:
                ...
                res = self.strat_e.execute(df, symbol, interval, strategy_filter)
    ...
    async def scan_cycle(self, session, symbols, interval, strategy_filter: Optional[List[str]] = None):
        label = f"[{'|'.join(strategy_filter)}|{interval}]" if strategy_filter else f"[all|{interval}]"

        sem = asyncio.Semaphore(self.cfg['api']['max_concurrent'])

        tasks = [
            self._proc_symbol(session, s, interval, sem, strategy_filter)
            for s in symbols
        ]
        ...
        try:
            ...
            logger.error(f"❌️ {label} UI engine state update failed: {ui_err}")

        self.notify_e.process_results(results, interval, label)
    ...
    async def interval_worker(self, session, strategy_names: List[str], interval: str):
        label = f"[{'|'.join(strategy_names)}|{interval}]"
        logger.info(f"🟢 {label} Worker started")
        last_run_slot = None

        while True:
            if not self.is_active:
                logger.critical(f"🛑 {label} System halted due to circuit breaker. Please restart manually.")
                break
            ...
            if wait_sec > 0:
                if wait_sec > 10:
                    ...
                    logger.info(f"💤 {label} Next alignment: {target_time} (waiting {int(wait_sec)}s)")
            ...
            try:
                ...
                symbols = (
                    [self.data_e.format_symbol(s) for s in watch_list]
                    if watch_list
                    else await self.data_e.get_active_symbols(session)
                )
                if not symbols:
                    reason = f"{label} Critical error: Failed to obtain active currency list (empty interface response)"
                    await self._trigger_circuit_breaker(interval, reason)
                    continue
                valid_results = await self.scan_cycle(
                    session, symbols, interval,
                    strategy_filter=strategy_names
                )
                if len(symbols) > 0 and (valid_results is None or len(valid_results) == 0):
                    reason = f"{label} Critical exception: All currency detail requests failed"
                    await self._trigger_circuit_breaker(interval, reason)
                    continue

                last_run_slot = current_slot
                logger.info(f"✅ {label} Scan done (valid: {len(valid_results)}), time: {time.time() - start_time:.2f}s")
            except Exception as e:
                logger.error(f"❌ {label} Runtime error: {e}", exc_info=True)
    ...
    async def run(self):
        async with aiohttp.ClientSession() as session:
            ...
            interval_strategy_map: Dict[str, List[str]] = {}
            for strategy_name, strategy_cfg in self.cfg['strategy']['router'].items():
                if not strategy_cfg.get('enabled'):
                    continue
                intervals = strategy_cfg.get('intervals') or self.cfg.get('intervals', [])
                for interval in intervals:
                    interval_strategy_map.setdefault(interval, []).append(strategy_name)
            ...
            try:
                ...
                if watch_list:
                    ...
                    logger.info(f"📋 Using watch list: {symbols}")
                else:
                    symbols = await self.data_e.get_active_symbols(session)

                if not symbols or len(symbols) == 0:
                    error_msg = "🚨 Program startup failed: symbol list is empty"
                    logger.critical(f"❌ {error_msg}")
                    await self.notify_e.send_error_msg(error_msg)
                    return

                if interval_strategy_map:
                    first_interval = next(iter(interval_strategy_map))
                    first_strategies = interval_strategy_map[first_interval]
                    await self.scan_cycle(
                        session, symbols, first_interval,
                        strategy_filter=first_strategies
                    )
            ...
            workers = []
            for interval, strategies in interval_strategy_map.items():
                workers.append(self.interval_worker(session, strategies, interval))
                logger.info(f"📌 Registered worker: {strategies} @ {interval}")
...
class UIEngine:
    ...
    def create_demo(self):
        with gr.Blocks() as demo:
            ...
            column_widths = ["100px", "60px", "120px", "80px", "150px", "60px", "100px", "120px", "80px", "120px", "240px",
                             "1600px",     ]
